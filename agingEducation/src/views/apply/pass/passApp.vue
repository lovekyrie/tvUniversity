    ...
    <div class="g-content g-content-footer">
      ...
      <div class="f-form">
        ...
        <p class="clearfix" v-for="(list,key) in formObj" v-if="key!='imgUrl'">
          <span class="f-span">{{msgdata[key]}}</span>
          <span>{{list}}：</span>
        </p>
        <div class="one-img clearfix" v-if="this.imglist.length>0">
          <div>
            <img :src="img" alt class="f-img" v-for="img in imglist">
          </div>
          <span>照片上传：</span>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    getListOne(item) {
      return new Promise((resolve, reject) => {
        this.until.get("/general/cat/getByCd", { cd: item.cd }).then(res => {
          if (res.status == 200) {
    ...
    getData() {
      this.until
        .get(`/px/clazzApply/info/${this.pxClazzApplyPk}`, {})
        .then(res => {
          if (res.status == 200) {
    ...
    payBtn() {
      /*验证是否勾选*/
      if (!this.checkbox) {
        this.$alert("请勾选已阅读并同意协议后，继续支付！", "提示", {
          ...
          if (res.status == 200) {
            ...
            window.open(
              `http://pic.nbtvu.net.cn:7009/NbtvuPI/GotoPay.aspx?profid=${
                msg.profId
              }&payinfoid=${msg.payInfoId}&paycode=${msg.payCd}&clienttype=pc`
            );
