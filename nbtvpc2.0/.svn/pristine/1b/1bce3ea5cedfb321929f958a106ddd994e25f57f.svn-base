    ...
    <div class="g-content">
      ...
      <div class="f-form">
        ...
        <div class="onelist clearfix" v-for="list in msglist">
          <input type="text" placeholder="单行输入" v-if="list.arg2=='text'" v-model="formmsg[list.arg1]">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      selectClass(cd){
        this.until.get('/general/cat/listByPrntCd',{prntCd:cd})
          .then(res=>{
            if(res.status == 200){
      ...
      payBtn() {
        ...
        this.formmsg.arg1 = this.courseName;
        ...
        this.until.postJson('/px/clazzApply/applyAdd',JSON.stringify(this.formmsg))
          .then(res=>{
            if(res.status == 400){
              this.$message.error(res.message);
            }else{
              window.location.href = './pass.html?pxClazzApplyPk=' + res.data.pxClazzApplyPk+'&pxClazzPk='+this.prntCd;
            }
          })
      }
    },
