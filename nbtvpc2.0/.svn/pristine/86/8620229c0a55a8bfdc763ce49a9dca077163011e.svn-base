        ...
        <div class="g-content g-content-footer">
            ...
            <div class="f-form">
                ...
                <div class="onelist clearfix">
                    <span>学生姓名：</span><span class="list-cnt">{{person.nm}}</span>
                </div>
                <div class="onelist clearfix">
                    <span>学员身份证号码：</span><span class="list-cnt">{{person.idCard}}</span>
                </div>
                <div class="onelist clearfix">
                    <span>学员学号：</span><span class="list-cnt">{{person.cd}}</span>
                </div>
                <div class="onelist clearfix">
                    <span>联系电话：</span><span class="list-cnt">{{person.mob}}</span>
                </div>
                <div class="f-msg" v-if="courseDList.length>0&&isOverCheck">
                    <div class="msg-tit"><p></p><span>未缴费</span></div>
                    <div v-for="(opt,index) in courseDList" class="bor-c msg-cnt"
                         v-if="opt.statCd == '30100.140'||opt.statCd == '30100.160'&&new Date(curTime)<new Date(opt.payEndTm)">
                        <p>课程名称： {{opt.arg1}}</p>
                        <p>缴费金额： <span>￥{{opt.payAmt}}</span></p>
                ...
                <div class="f-msg" style="margin-bottom: 90px;" v-if="courseYList.length>0">
                    <div class="msg-tit"><p></p><span style="color:black;">已缴费</span></div>
    ...
    methods: {
      ...
      getKpay() {
        return new Promise(resolve => {
          ...
          this.until.get('/px/clazzApply/list', courseParam)
            .then(res => {
              if (res.status == 200) {
      ...
      getCode(obj) {
        return new Promise((resolve, rej) => {
          this.fullscreenLoading = true;
          this.until.post('/px/clazzApply/czPay', {clazzApplyPk: obj.pxClazzApplyPk})
            .then(res => {
              this.fullscreenLoading = false;
              if (res.status == 200) {
                resolve(res.data);
      ...
      queryCode(obj) {
        let $q = new Promise((resolve, reject) => {
          ...
          this.until.post('/px/clazzApply/updPayState', param)
            .then(res => {
              if (res.status == 200) {
                resolve('ok');
              }
      ...
      getPerson() {
        return new Promise(resolve => {
          ...
          this.until.get('/px/stu/list', msgParam)
            .then(res => {
              if (res.status == 200) {
                /*检测身份证是否存在*/
                ...
                if (res.data.items[0].statNm == '否') {
                  resolve('false');
                  this.$alert('系统未检测到您的学籍信息，您可能未注册或未通过学籍审核，如有疑问请您联系学校。', '警告', {
                    confirmButtonText: '确定',
                    callback: action => {
                      history.back();
                    }
      ...
      delOldTime() {
        return new Promise(resolve => {
          for (let index = 0; index < this.courseDList.length; index++) {
            if (new Date(this.curtime) > new Date(this.courseDList[index].payEndTm)) {
      ...
      async openURL(e, opt, index) {
        ...
        console.log(opt);
        window.open(`http://pic.nbtvu.net.cn:7009/NbtvuPI/GotoPay.aspx?profid=${opt.profId}&payinfoid=${opt.payInfoId}&paycode=${opt.payCd}&clienttype=pc`);
      ...
      cancelBtn(pxClazzApplyPk) {
        this.$confirm('是否取消该门课的报名?', '提示', {
          ...
          this.until.post('/px/clazzApply/cancelApply', {pxClazzApplyPk})
            .then(res => {
              if (res.status == 200) {
