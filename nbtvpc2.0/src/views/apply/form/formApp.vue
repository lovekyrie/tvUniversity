    ...
    methods: {
      ...
      getList() {
        return new Promise((resolve, reject) => {
          this.until.get(`/px/clazzApplyConf/getApplyConf`, {pxClazzPk:this.pxClazzPk})
            .then(res => {
              if (res.status == 200) {
      ...
      upImg() {
        ...
        for (var j = 0; j < file.length; j++) {
          if (file[j].size > maxsize) {
            this.$alert('图片大小请限制在2M以内！', '提示', {confirmButtonText: '确定'});
            this.$refs[el].value = '';
            return;
          }
        }
      ...
      payBtn() {
        ...
        /*验证是否勾选*/
        if (!this.checkbox) {
          this.$alert('请勾选已阅读并同意协议后完成支付', '提示', {confirmButtonText: '确定'});
          return;
        }
