class until{
  ...
  //获取token
  getTokenData(){
    return JSON.parse(sessionStorage.getItem('DD_token'));
  }
  getToken(){
    return JSON.parse(sessionStorage.getItem('DD_token')).token;
  }
  /*一寸照在这个PK里*/
  getStuPK(){
    return JSON.parse(sessionStorage.getItem('DD_token')).stu.pxStuPk;
  }
  getSysUserPK(){
    return JSON.parse(sessionStorage.getItem('DD_token')).stu.sysUserPk;
  }
  /*删除token*/
  delToken(){
  ...
  isLogin(){
    return sessionStorage.getItem('DD_token')?true:false;
  }
  //截取URL参数
  getQueryString(name) {
