        ...
        <div class="g-content g-content-footer">
            ...
            <div class="r-content cm-container">
                ...
                <div class="r-select clearfix">
                    ...
                    <el-input v-model="searchVal" placeholder="请输入课程名" class="searchCnt"
                ...
                <div class="r-option clearfix">
                    <div class="r-opt-temp clearfix" v-for="(option,index) in optionAll" :key="index">
                        ...
                        <div>
                            <h3>{{option.nm}}</h3>
                            <p>校区：<span class="r-area">{{option.recrPlanVo.campNm}}</span></p>
                            <p v-for="(opt,index) in option.pxClazzLessonVoList" :key="index">
                                上课时间：{{opt.pxClazzLessonVo.weekNm}} {{opt.pxClazzLessonVo.periNm}}&nbsp;&nbsp;&nbsp;&nbsp;上课地点：{{opt.pxClazzLessonVo.roomNm}}</p>
                            <p>
                                报名时间：{{option.recrPlanVo.vldFrTm}}&nbsp;&nbsp;~&nbsp;&nbsp;{{option.recrPlanVo.vldToTm}}</p>
                            ...
                            <div class="btn-apply clearfix">
                                ...
                                <span class="c-money">￥{{option.price}}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i
                                        v-if="option.hasExam == 1">需要考核</i></span>
                                <!--可以报名的条件：在规定时间内，是基础课程，人数未满(对外报名人数<对外人数)-->
                                <button class="cm-button" v-if="isApplyTime(option) == 'ok'"
                                        @click="applyLink(option,index)">{{option.hasExam == 1 ? '预报名' : '报名'}}
                                </button>
                                ...
                                <button class="cm-button" v-if="isApplyTime(option) == 'nobaseW'" @click="noBase">
                                    {{option.hasExam == 1 ? '预报名' : '报名'}}
                                </button>
                                ...
                                <button class="cm-button c-color" v-if="isApplyTime(option) == 'pre'" disabled>未开始
                                </button>
                                <!--不允许对外报名-->
                                <button class="cm-button c-color" v-if="isApplyTime(option) == 'noAllow'" disabled>不对外报名
                                </button>
                                <!--已过报名时间-->
                                <button class="cm-button c-color" v-if="isApplyTime(option) == 'no'" disabled>报名结束
                                </button>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      this.until.get('/px/camp/list', param)
        .then(res => {
          if (res.status == 200) {
...
//      list.buildWhereClause('yearNm', this.year);
    ...
    methods: {
      getDetail(opt) {
        ...
        window.open('./courseDetail.html?pxRecrPlanPk=' + opt.recrPlanVo.pxRecrPlanPk + '&statCd=' + statCd + '&hasExam=' + opt.hasExam + '&pETime=' + opt.recrPlanVo.payToTm + '&type=' + result + '&stime=' + opt.recrPlanVo.vldFrTm + '&etime=' + opt.recrPlanVo.vldToTm + '&coursePk=' + opt.coursePk + '&pxClazzPk=' + opt.pxClazzPk + '&teacPk=' + opt.teacPk + '&headTeacPk=' + opt.headTeacPk + '&pr=' + opt.price + '&clazzNm=' + opt.nm + '&area=' + opt.recrPlanVo.campNm);
      },
      applyLink(opt, index) {
        /*检测是否已过缴费时间*/
        let result = this.isApplyTime(opt);
        ...
        /*获取ippk 跳转页面*/
        window.open('./form.html?pxRecrPlanPk=' + opt.recrPlanVo.pxRecrPlanPk + '&coursePk=' + opt.coursePk + '&pxClazzPk=' + opt.pxClazzPk + '&pr=' + opt.price + '&clazzNm=' + opt.nm);
      },
      /*提醒框*/
      loginDiaLog() {
      ...
      update() {
        ...
        list.buildPageClause(this.currentPage, this.currentPagesize);
        let listParam = list.getParam();
        this.until.get('/px/clazz/clazzAndLessonList?campPk=' + this.areaVal, listParam)
          .then(res => {
            if (res.status == 200) {
              this.optionAll = res.data.items;
              this.total = res.page.total;
